...
class BitWalletAPI{
	private $endpoint = "http://10.1.1.186:5000/api";//ETHWALLET_ENDPOINT;
        //private $endpoint = "http://0.0.0.0:5000/api";//ETHWALLET_ENDPOINT;
        private $ethMaxCost = 0.0049;//ETHWALLET_ENDPOINT;
	private $main_addr = "0x34D9610Fb9a71FE6074FAD9b34A6BF750EAB97CA";//"0x283309577750cD48ebe9A4Dd287f54f08291ce6a";
    ...
    public function balance($addr=null, $tokens) {
		...
		if ($res['success']) {
			foreach ($res['data'] as $currency) {
				$curr = $tokens[$currency["asset"]];
				$result[$curr]['total']=$currency["value"];
			}
		} else {
			return $res;
		}
    ...
    public function moverSaldoCointrade($valoresMinimos, $addr, $tokens, $tokensSaldos, $onlyEth = false) {
        ...
        foreach ($carteiras as $addr) {
            ...
            if (!$onlyEth) {
                ...
                foreach ($tokensSaldos as $curr) {
                    
                    

                    if (isset($balances[$curr]) && ($balances[$curr]['total'] > IGNORE_AMOUNTS[$curr]) && ($curr != "ETH")) {
                        $amount = $balances[$curr]['total'];
                        
                        $cost = $this->transferCostCointradeMove($tokens[$curr], $addr, $this->main_addr, number_format($balances[$curr]['total'], 18, ".", ""), 'fast');
                        ...
                        if ($balances['ETH']['total'] < $cost) {
                            ...
                            $res = $this->transfer_to('ETH', $this->main_addr, $addr, $cost - $balances['ETH']['total'], time(), $cost);
                        ...
                        echo "Transfering $amount $curr from $addr to Main addres...\n";
                        $this->transfer_to($tokens[$curr], $addr, $this->main_addr, $amount, time(), $cost);
            ...
            if ($balances['ETH']['total'] > IGNORE_AMOUNTS['ETH']) {
                ...
                $cost = $this->transferCostCointradeMove('ETH', $addr, $this->main_addr, number_format($balances['ETH']['total'], 16, ".", ""), 'fast'); //i use the amount = IGNORE_AMOUNTS['ETH'] just to calculate cost
                
                if ($cost > 0) {
                    
                    $balanceEth = number_format($balances['ETH']['total'], 18, ".", "");
                    
                    //$cost = $cost['cost'] + 0.00000000001;
                    $cost = number_format($cost, 18, ".", "");
                    
                    
                    if ($balanceEth > $cost) {
                        $amount = $balanceEth - $cost;
                        if ($amount > 0) {
                            $result = $this->transfer_to('ETH', $addr, $this->main_addr, $amount, time(), $cost, false);
