...
class BookWeb {
        ...
        try {             
            ...
            if($cliente->modoOperacao == "basic"){
                ...
                $params["venda"] = number_format($taxas["venda"], 4, ".", "");
                 ...
                 \Utils\Layout::view("book_web", $params);
            } else {
            ...
            foreach ($paridades as $paridade) {
                ...
                <tr class="<?php echo ($favorita ? "favorite-parity" : "") ?> tr-h" data-paridade="<?php echo \Utils\Criptografia::encriptyPostId($paridade->id)?>" >                    
                    ...
                    <td class="text-right change-parity column-paridade" style="vertical-align: middle; padding-top: 1px !important; padding-bottom: 1px !important; width: 35% !important" data-name="<?php echo $paridade->symbol ?>">
                        <?php echo number_format($paridade->quoteVolume, $paridade->moedaTrade->casasDecimais, ",", ".") ?> <?php echo $paridade->moedaTrade->simbolo ?>
                    </td>
                    <?php } ?>
    ...
    public function salvarOrdemCompra($params) {
        try {
            ...
            //exit("Volume: " . $params["_POST"]["amount"] . " - Price: " . $params["_POST"]["price"] . " -  Total: " . $params["_POST"]["amount"] * $params["_POST"]["price"]);
            ...
            //exit("Volume: " . 0.05333333333333 . " - Price: " . 37500.0000 . " -  Total: " . 0.05333333 * 37500.00000000 . " Div: " . floatval(2000 / 37500.0000) . " - BCMUL: " . bcmul("37500", "0.053333333333", 4));
    ...
    public function getBook($params) {
        ...
        try {
            ...
            $listaCompra = $orderBookRn->getOrders($paridade, \Utils\Constantes::ORDEM_COMPRA, "N", "N", 17, 0, false);
            $listaVenda = $orderBookRn->getOrders($paridade, \Utils\Constantes::ORDEM_VENDA, "N", "N", 16, 0, false);
            ...
            $json["diferenca"] = $paridade->moedaTrade->simbolo . " " . number_format($diferenca, $paridade->moedaTrade->casasDecimais, ".", "");
    ...
    private function htmlItemOrdem(\Models\Modules\Cadastro\OrderBook $ordem, $volumeGeral, $idCliente, $volumeAcumulado) {
        ...
        <tr style="background-image: linear-gradient(to left, rgba(<?php echo $color ?>, 0.2) <?php echo number_format($ordem->porcentagem, 2, ".", "") ?>%, rgba(<?php echo $color ?>, 0) <?php echo number_format($ordem->porcentagem, 2, ".", "") ?>%) !important; font-weight: <?php echo $ordem->idCliente == $idCliente ? "bold" : "normal" ?>"    
            class="order-item" 
            onclick="configOrder('<?php echo ($ordem->tipo == \Utils\Constantes::ORDEM_COMPRA ? \Utils\Constantes::ORDEM_VENDA : \Utils\Constantes::ORDEM_COMPRA) ?>', '<?php echo number_format($ordem->valorCotacao, $ordem->paridade->moedaTrade->casasDecimais, ".", "")?>', '<?php echo number_format($volumeAcumulado, $ordem->paridade->moedaBook->casasDecimais, ".", "")?>');" >
            
            <td class="text-left td-price" style="padding-top: 1px !important; padding-bottom: 1px !important; color: <?php echo $colorHex ?>; ">
                <span><?php echo number_format($ordem->valorCotacao, $ordem->paridade->moedaTrade->casasDecimais, ",", ".") ?>&nbsp;&nbsp;&nbsp;&nbsp;</span>
            ...
            <td class="text-right " style="padding-top: 1px !important; padding-bottom: 1px !important;"><?php echo number_format(($ordem->valorCotacao * $ordem->volumeCurrency), $ordem->paridade->moedaTrade->casasDecimais, ",", ".") ?></td> 
    ...
    private function htmlItemMinhasOrdens(\Models\Modules\Cadastro\OrderBook $ordem) {
        ...
        ?>
            <tr class="my-order-item tr-h" style="<?php echo $color?>;">
                ...
                <td class="text-center"><?php echo $tradeSymbol ?> <?php echo number_format(($ordem->valorCotacao * $ordem->volumeCurrency), $ordem->paridade->moedaBook->casasDecimais, ",", ".")?></td>
    ...
    public function getBalance() {
        ...
        try {
            ...
            $saldos = \Models\Modules\Cadastro\SaldoClienteRn::getSaldo($cliente->id);
            
            $saldoBook = 0;
            $saldoTrade = 0;
            
            foreach ($saldos as $s) {
                if ($s["id"] == $paridade->idMoedaBook) {
                    $saldoBook = $s["saldo"];
                } else if ($s["id"] == $paridade->idMoedaTrade) {
                    $saldoTrade = $s["saldo"];
                }
            }
            
            $json["book"] = number_format($saldoBook, $paridade->moedaBook->casasDecimais, ",", "");
            $json["bookd"] = number_format($saldoBook, $paridade->moedaBook->casasDecimais, ".", "");
            $json["trade"] = number_format($saldoTrade, $paridade->moedaTrade->casasDecimais, ",", "");
            $json["traded"] = number_format($saldoTrade, $paridade->moedaTrade->casasDecimais, ".", "");
            $json["moedaBook"] = $paridade->moedaBook->simbolo;
            $json["moedaTrade"] = $paridade->moedaTrade->simbolo;
            $json["sucesso"] = true;
        } catch (\Exception $ex) {
            $json["sucesso"] = false;
            $json["mensagem"] = \Utils\Excecao::mensagem($ex);
