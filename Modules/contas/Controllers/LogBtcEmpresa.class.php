...
class LogBtcEmpresa {
    ...
    public function index($params) {
        ...
        \Utils\Layout::view("log_btc_empresa", $params);
    }
    ...
    public function filtrar($params) {
        try {
            ...
            $lista = $logContaCorrenteBtcEmpresaRn->filtrar($dataInicial, $dataFinal, $filtro, $idUsuario);
                ...
                foreach ($lista as $logContaCorrente) {
                    ...
                    <li class="list-group-item" style="font-size: 10px; font-weight: bold;">
                        <div class="row">
                            ...
                            <div class="col-lg-1 text-center">
                                <?php echo $logContaCorrente->idContaCorrenteBtcEmpresa ?>
                            </div>
                        </div>
                    </li>
                    <?php
    ...
    public function imprimir($params) {
        try {
            ...
            $pdf = new \Utils\PDF();
            ob_start();
            \Utils\Layout::view("impressos/extrato_log_btc_empresa", $params);
            $html = ob_get_contents();
            ob_end_clean();
            $pdf->conteudo($html);
            
            
            $pdf->gerar("extrato_logs_conta_corrente_btc_empresa.pdf", "D", false, false, false);
        } catch (\Exception $ex) {
            ?>
            <html>
                <head>
