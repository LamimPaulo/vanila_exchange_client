...
class LogBtc {
        ...
        if (!(\Utils\Geral::isUsuario() && \Utils\Geral::getLogado()->tipo == \Utils\Constantes::ADMINISTRADOR)) {
            ...
            if (isset($get[0])) {
                ...
                if (\Utils\Geral::getCliente()->id !== $id) {
                    \Utils\Geral::redirect(URLBASE_CLIENT."contas/btc/lancamentos/".\Utils\Geral::getCliente()->id);
                }
            }
        }
    }
    ...
    public function index($params) {
        ...
        if (!$adm) {
            ...
            \Utils\Layout::view("log_btc", $params);
        }
    }
    ...
    public function filtrar($params) {
        try {
            ...
            $logContaCorrenteBtcRn = new \Models\Modules\Cadastro\LogContaCorrenteBtcRn();
            $lista = $logContaCorrenteBtcRn->filtrar($idCliente, $dataInicial, $dataFinal, $filtro, $idUsuario);
            ...
            if (sizeof($lista) > 0) {
                ...
                foreach ($lista as $logContaCorrente) {
                    //$logContaCorrente = new \Models\Modules\Cadastro\LogContaCorrenteBtc();
                    ...
                    <li class="list-group-item" style="font-size: 10px; font-weight: bold;">
                        <div class="row">
                            ...
                            <div class="col-lg-1 text-center">
                                <?php echo $logContaCorrente->idContaCorrenteBtc ?>
                            </div>
                        </div>
                    </li>
                    <?php
    ...
    public function imprimir($params) {
        try {
            ...
            $pdf = new \Utils\PDF();
            ob_start();
            \Utils\Layout::view("impressos/extrato_log_btc", $params);
            $html = ob_get_contents();
            ob_end_clean();
            $pdf->conteudo($html);
            
            
            $pdf->gerar("extrato_logs_conta_corrente_btc.pdf", "D", false, false, false);
        } catch (\Exception $ex) {
            ?>
            <html>
                <head>
