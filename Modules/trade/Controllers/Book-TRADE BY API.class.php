...
class Book {
    ...
    public function salvarOrdemCompra($params) {
        try {
            ...
            if($cliente->id == 15093064543895){
                
                $result = \Cointrade\Trade::comprar($cliente, $paridade->symbol, $amount, $price, true);
                
                if(!empty($result)){
                    
                    if($result->success){
                        $sucesso = true;
                        $mensagem = $this->idioma->getText("ordemRegistradaSucessoC");
                    } else {
                        $sucesso = false;
                    ...
                    $sucesso = false;
                    $mensagem = "Falha para registrar a ordem.";
                }
            } else {
        ...
        try {            
            ...
            if($cliente->id == 15093064543895){
                ...
                if(!empty($result)){
                    ...
                    if($result->success){
                        $sucesso = true;
                        $mensagem = $this->idioma->getText("ordemRegistradaSucessoV");
                    } else {
                        $sucesso = false;
                        $mensagem = $result->message;
                    }
    ...
    public function getBalance() {
        ...
        try {
            ...
            $json["trade"] = number_format($saldoTrade, $this->casasDecimaisMoedaTrade, ",", "");
            $json["traded"] = number_format($saldoTrade, $this->casasDecimaisMoedaTrade, ".", "");
